...
class DownsamplerBlock (nn.Module):
    def __init__(self, ninput, noutput):
        ...
        self.pool = nn.MaxPool2d(2, stride=2)  #2x2
...
class Decoder (nn.Module):
    def __init__(self, num_classes):
        ...
        self.layers = nn.ModuleList()#ModuleList是Module的子类, 被设计用来存储任意数量的nn. module。
...
class ERFNet(nn.Module):
    ...
    def forward(self, input, only_encode=False):
        if only_encode: #如果仅仅训练编码 返回encoder的前向传播only_encode=False 默认训练整个encoder+decoder
...
if __name__ == '__main__':
    ...
    model = ERFNet(classes=2).to(device)
