...
class CGNet(nn.Module):
    ...
    """
    def __init__(self, classes=2, M= 3, N= 21, dropout_flag = False):
        """
        args:
          classes: number of classes in the dataset. Default is 2 for the pv dataset
...
if __name__ == '__main__':
    ...
    # Define the model
    model = CGNet(classes=2).to(device)

    # Display model summary
    summary(model, (3, 360, 360))

    # Create a dummy input tensor (e.g., a batch of 1 image with 3 channels of 360x360)
    input_tensor = torch.randn(1, 3, 360, 360).to(device)
    ...
    num_iterations = 100  # Run for 100 iterations for a more accurate average FPS
