...
class LETNet(nn.Module):
    def __init__(self, classes=2, block_1=3, block_2=12, block_3=12, block_4=3, block_5 = 3, block_6 = 3):
        super().__init__()
...
#Transformer
        ...
        lc3_output = self.LC3(output3)
        lc3_output = F.interpolate(lc3_output, size=output4.shape[2:], mode='bilinear', align_corners=False)
        output4 = self.upsample_1(output4 + lc3_output)

        output4 = self.bn_prelu_5(output4)

        output5 = self.DAB_Block_5(output4)
        lc2_output = self.LC2(output2)
        lc2_output = F.interpolate(lc2_output, size=output5.shape[2:], mode='bilinear', align_corners=False)
        output5 = self.upsample_2(output5 + lc2_output)

        output5 = self.bn_prelu_6(output5)

        output6 = self.DAB_Block_6(output5)
        lc1_output = self.LC1(output1)
        lc1_output = F.interpolate(lc1_output, size=output6.shape[2:], mode='bilinear', align_corners=False)
        output6 = self.upsample_3(output6 + lc1_output)
