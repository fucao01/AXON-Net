...
class DriveDataset(Dataset):
    """
    Simple segmentation dataset loader.
    Expected structure:
      <root>/TP-Dataset/JPEGImages/*.jpg
      <root>/TP-Dataset/GroundTruth/*.png
      <root>/TP-Dataset/Index/{train.txt,val.txt}
    Each line in the index files is a filename stem without extension.
    """
    ...
    def __init__(self, root: str, transforms=None, txt_name: str = "train.txt"):
        ...
        if not os.path.exists(data_root):
            raise FileNotFoundError(f"path '{data_root}' does not exist.")
        ...
        if len(self.images) != len(self.masks):
            raise RuntimeError("image/mask list length mismatch.")
...
def cat_list(images: List, fill_value: int = 0):
